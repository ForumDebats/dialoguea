...
var
	...
	mongoose = require(loadmodule('mongoose')),
...
exports.Commentaire = new Schema({
...
function setFieldsRequiredByDefautlt(schema) {
	var schs;
	if (!schema.length) //single object
		schs = [schema]
	else schs = schema;
	for (var j= 0, _l=schs.length; j<_l; j++) {
		schema = schs[j]
		var attr
		for (var i= 0,_L=schema.paths.length; i<_L; i++) {
			attr = schema.paths[i]
